...
const AdminBanner = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/admin/banners?page=${currentPage}&limit=${itemsPerPage}`);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put(`http://localhost:3000/admin/banner/${formData.id}`, form, {
  ...
  return (
    <div className="main">
      <div className="topbar">
        <div className="toggle"><ion-icon name="menu-outline"></ion-icon></div>
        <div className="search">
          <label><input type="text" placeholder="Tìm kiếm" /><ion-icon name="search-outline"></ion-icon></label>
        </div>
        <div className="user"><img src="/images/user.jpg" alt="User" /></div>
      </div>
          ...
          <table>
            ...
            <tbody>
              {banners.map((banner) => (
                <tr key={banner.id}>
                  <td>{banner.id}</td>
                  <td><img src={banner.image} alt="Banner" style={{ width: "120px" }} /></td>
          ...
          {totalBanners > itemsPerPage && (
            <div className="paginationContainer">
              <ReactPaginate
                ...
                forcePage={Math.min(Math.max(0, currentPage - 1), Math.ceil(totalBanners / itemsPerPage) - 1)}
              />
            </div>
          )}
